...
export const generateMetadata = async ({ params }: Props): Promise<Metadata> => {
	try {
		const { id, name } = await getPokemonByName((await params).name);
...
export const generateStaticParams = async () => {
	...
	const { results }: PokemonsResponse = await response.json();
...
export default async function PokemonsNamePage({ params }: Props) {
	...
	const pokemon = await getPokemonByName((await params).name);
